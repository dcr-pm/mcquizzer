...
const ScoreScreen: React.FC<ScoreScreenProps> = ({ stats, player, onPlayAgain, onNewCategory, onShowLeaderboard }) => {
  ...
  return (
    <>
      <div className="text-center p-4 sm:p-8 flex flex-col items-center justify-center h-full animate-fade-in-up">
        <div className="bg-white/10 backdrop-blur-md p-6 sm:p-10 rounded-2xl shadow-2xl border border-white/20 w-full max-w-2xl">
          <i className="fa-solid fa-check-circle text-5xl sm:text-6xl text-green-400 mb-4"></i>
          <h2 className="text-3xl sm:text-4xl font-bold text-white mb-2">Quiz Complete!</h2>
          <p className="text-lg sm:text-xl text-gray-300 mb-6">{getPerformanceMessage()}</p>

          <div className="bg-gray-900/50 p-4 sm:p-6 rounded-lg mb-8 text-left grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                  <p className="text-xs sm:text-sm text-gray-400">Category</p>
                  <p className="text-xl sm:text-2xl font-bold text-blue-300">{categoryName}</p>
              </div>
              <div>
                  <p className="text-xs sm:text-sm text-gray-400">Questions Answered</p>
                  <p className="text-xl sm:text-2xl font-bold text-white">{totalQuestions}</p>
              </div>
              <div>
                  <p className="text-xs sm:text-sm text-gray-400">Correct Answers</p>
                  <p className="text-xl sm:text-2xl font-bold text-green-400">{correctAnswers} / {totalQuestions}</p>
              </div>
              <div>
                  <p className="text-xs sm:text-sm text-gray-400">Points Earned</p>
                  <p className="text-xl sm:text-2xl font-bold text-yellow-400">+{pointsEarned.toLocaleString()} pts</p>
              </div>
          </div>
